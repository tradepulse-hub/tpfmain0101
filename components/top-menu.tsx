...
// Atualizar a interface TopMenuProps para incluir "partnerships"
...
export function TopMenu({ activeTab = "agenda" }: TopMenuProps) {
  ...
  return (
    <>
      ...
      >
        <motion.button
          onClick={toggleMenu}
          className="w-12 h-12 rounded-full bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center shadow-lg border border-gray-700/50 relative overflow-hidden backdrop-blur-sm"
        ...
        >
          ...
          <motion.div animate={{ rotate: menuOpen ? 180 : 0 }} transition={{ duration: 0.3, ease: "easeInOut" }}>
            {menuOpen ? <X className="w-5 h-5 text-gray-200" /> : <Menu className="w-5 h-5 text-gray-200" />}
          </motion.div>
        </motion.button>
      </motion.div>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            <div className="max-w-md mx-auto p-6 h-full pt-20">
              {/* Menu header */}
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center space-x-3">
                  <div className="w-8 h-8 relative">
                    <Image src="/logo-tpf.png" alt="TPulseFi" width={32} height={32} className="w-full h-full" />
                  </div>
                  <div className="text-white text-lg font-semibold">TPulseFi</div>
                </div>
                <div className="text-gray-400 text-sm font-medium">{translations.nav?.menu || "Menu"}</div>
              </div>

              {/* Quick Navigation - Principais ações */}
              <div className="mb-8">
                <div className="text-gray-400 text-sm font-medium mb-4">Navegação Rápida</div>
                <div className="grid grid-cols-4 gap-3">
                  {[
                    { name: "Agenda", icon: "calendar", path: "/agenda", active: activeTab === "agenda" },
                    { name: "Carteira", icon: "wallet", path: "/wallet", active: activeTab === "wallet" },
                    { name: "Perfil", icon: "profile", path: "/profile", active: activeTab === "profile" },
                    { name: "Vencedores", icon: "trophy", path: "/winners", active: activeTab === "winners" },
                  ].map((item) => (
                    <Link href={item.path} key={item.name} onClick={() => setMenuOpen(false)}>
                      <motion.div
                        className={`flex flex-col items-center justify-center p-3 rounded-xl cursor-pointer relative overflow-hidden group ${
                          item.active ? "bg-blue-500/20 border border-blue-500/30" : ""
                        }`}
                      ...
                      >
                        ...
                        <span className="relative z-10 text-xs text-gray-300 group-hover:text-white transition-colors duration-300 text-[10px] text-center">
                          {item.name}
                        </span>
                      </motion.div>
                    </Link>
              ...
              {/* Menu grid - Outras opções */}
              <div>
                <div className="text-gray-400 text-sm font-medium mb-4">Mais Opções</div>
